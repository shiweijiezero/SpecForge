...
from models.modeling_diffusion import DiffusionTransformer
...
class ModelArguments:
    ...
    """
    ae_name_or_path: Optional[str] = field(
        default=None,
        metadata={
            "help": (
...
def main():
    ...
    config.ae_path = model_args.ae_name_or_path

    if config.model_type == 'energy':
        model_class = EnergyTransformer
    elif config.model_type == 'diffusion':
        model_class = DiffusionTransformer
    elif config.model_type == 'flow':
        model_class = FlowTransformer
    else:
