...
echo "======================================================"
echo "Medusa Training Pipeline for Qwen2.5 7B"
echo "======================================================"
echo "包含步骤:"
echo "  1. 下载模型和数据集"
...
python scripts/generate_data_by_target.py \
    ...
    --output-dir /tmp/generated-dataset/qwen2.5-7b-instruct \
    --max-concurrency 512 \
    --num-per-shard 50000 \
...
python scripts/merge_shards.py \
    --input-dir /tmp/generated-dataset/qwen2.5-7b-instruct \
...
echo ""
echo "Qwen2.5特有参数:"
echo "  Chat Template:   qwen  ← Qwen模型专用"
echo "  Draft Vocab:     16000 ← Qwen词表更大，draft用16k"
echo ""
...
torchrun \
    ...
    --train-data-path /tmp/dataset/train.jsonl \
    --eval-data-path /tmp/dataset/eval.jsonl \
    --output-dir /tmp/outputs/qwen25-7b-medusa \
    --cache-dir /tmp/cache \
    --num-epochs 1 \
    --batch-size 1 \
    --learning-rate 5e-5 \
...
echo "✅ 训练完成！"
echo "======================================================"
echo "模型保存至: /tmp/outputs/qwen25-7b-medusa/final"
echo ""
echo "SGLang推理命令:"
echo "  python -m sglang.launch_server \\"
echo "    --model-path Qwen/Qwen2.5-7B-Instruct \\"
echo "    --speculative-draft-model-path /tmp/outputs/qwen25-7b-medusa/final \\"
